...
import com.gstcalculator.domain.model.GenerationProfile
import kotlin.random.Random

object GSTCalculatorGenerator {
    ...
    fun generate(
        ...
        repeat(64) {
            ...
            if (GSTCalculatorEngine.validateLevel(level)) return level
            attemptSeed++
        }
    ...
    fun generateForChallenge(
        ...
        levelNumber: Int,
        difficulty: Difficulty
    ): GSTCalculatorLevel = generate(seed, levelNumber, difficulty)
    ...
    private fun buildLevel(
        ...
        val capacity = GSTCalculatorLevel.DEFAULT_CAPACITY
    ...
    private fun scrambleFromSolved(
        ...
        while (made < moves && attempts < moves * 20) {
            ...
            if (from == to) continue
            val game = GSTCalculatorEngine.createInitialGame(
                GSTCalculatorLevel(
                    seed = 0,
                    levelNumber = 1,
            ...
            if (!GSTCalculatorEngine.canPour(game, from, to)) continue
            val updated = GSTCalculatorEngine.pour(game, from, to)
