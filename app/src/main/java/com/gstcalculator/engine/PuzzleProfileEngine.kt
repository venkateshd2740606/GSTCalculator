...
import com.gstcalculator.domain.model.PuzzleArchetype
import com.gstcalculator.domain.model.PuzzleProfile
import com.gstcalculator.domain.model.PuzzleProfileMetrics
import com.gstcalculator.domain.model.GSTCalculatorGame
import com.gstcalculator.domain.model.SkillCategory
...
object PuzzleProfileEngine {
    ...
    fun updateMetrics(current: PuzzleProfileMetrics, game: GSTCalculatorGame): PuzzleProfileMetrics {
        if (game.status != GameStatus.COMPLETED || game.level.isTutorial) return current

        val moveCount = max(game.moves, 1)
        val optimalMoves = GSTCalculatorEngine.optimalMoveCount(game)
