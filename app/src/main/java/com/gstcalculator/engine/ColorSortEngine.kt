...
object GSTCalculatorEngine {
    ...
    fun createInitialGame(level: GSTCalculatorLevel): GSTCalculatorGame = GSTCalculatorGame(
        level = level,
    ...
    fun pourCount(game: GSTCalculatorGame, from: Int, to: Int): Int {
    ...
    fun canPour(game: GSTCalculatorGame, from: Int, to: Int): Boolean = pourCount(game, from, to) > 0

    fun pour(game: GSTCalculatorGame, from: Int, to: Int): GSTCalculatorGame {
    ...
    fun onTubeSelected(game: GSTCalculatorGame, tubeId: Int): GSTCalculatorGame {
    ...
    fun isWon(tubes: List<List<Int>>, level: GSTCalculatorLevel): Boolean =
        tubes.all { tube -> tube.isEmpty() || tube.distinct().size == 1 }

    fun isWon(game: GSTCalculatorGame): Boolean = isWon(game.tubes, game.level)

    fun getHintMove(game: GSTCalculatorGame): Pair<Int, Int>? {
    ...
    fun solve(game: GSTCalculatorGame): List<Pair<Int, Int>>? {
        if (isWon(game)) return emptyList()

        data class Node(val state: GSTCalculatorGame, val path: List<Pair<Int, Int>>)
    ...
    fun optimalMoveCount(game: GSTCalculatorGame): Int =
